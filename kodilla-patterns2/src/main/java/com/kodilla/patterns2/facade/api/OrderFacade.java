...
public class OrderFacade {
    ...
    public void processOrder(final OrderDto order, final Long userId)
        ...
        LOGGER.info("Registring new order, Id: " + orderId);
        if (orderId < 0) {
        ...
        try {
            for (ItemDto orderItem : order.getItems()) {
                LOGGER.info("Adding value" + orderItem.getProductId() + ", " +
            ...
            if (!shopService.verifyOrder(orderId)) {
                ...
                LOGGER.error(OrderProcessingException.ERR__VERIFICATION_ERROR);
                wasError = true;
                throw new OrderProcessingException(OrderProcessingException.ERR__VERIFICATION_ERROR);
            }
